...
- Share payload is the bare result text (`4, 6 (sum: 10)`, `Pizza (35% chance)`), no tab-name prefix and no page URL — a `PAGES_BASE_PATH`-dependent URL would differ between dev and the deployed site. Each tab builds that string from the same state the result block renders, so what is shared always matches what is on screen (this matters for `DiceRoll`, which keeps `randomValues` in state set only inside `handleRoll`, never recomputed from `Math.random()` during render).
