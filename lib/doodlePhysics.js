...
// most — so N^2 is comfortably cheap). Overlapping pairs that share a color
// or a shapeType merge (unless the merged size would exceed MAX_MERGE_SIZE);
// every other overlapping pair bounces elastically. A merged shape is
// consumed for the rest of this pass — it can't also collide with a third
// shape in the same frame; that's resolved next frame instead.
//
...
// not physics.
//
// rng (optional): forwarded to mergeShapes for its shapeType/color coin
// flips — same rng-threading convention as every other randomized doodle
// helper (createShape, splitShape).
// eslint-disable-next-line import/prefer-default-export
export function resolveCollisions(shapes, grabbedIds = null, rng = Math.random) {
    ...
    if (!removed.has(a.id)) {
      for (let j = i + 1; j < working.length; j += 1) {
        ...
        if (!removed.has(b.id)) {
          ...
          // entirely for its grace window, so siblings spawned near the
          // same point can disperse before merge/bounce logic sees them.
          ...
          if (dist < minDist && !aGraced && !bGraced) {
            ...
            const sameFamily = a.color === b.color || a.shapeType === b.shapeType;
            if (sameFamily && !aGrabbed && !bGrabbed) {
              const combinedSize = Math.sqrt(a.size ** 2 + b.size ** 2);
              // MAX_MERGE_SIZE is defined at sizeMultiplier 1; scale it by
              // the larger shape's own multiplier so tablet-spawned shapes
              // (already up to 2x bigger) aren't stuck bouncing off every
              // same-family neighbor because the fixed base cap was already
              // smaller than their own resting size.
              const multiplier = Math.max(a.sizeMultiplier || 1, b.sizeMultiplier || 1);
              if (combinedSize <= MAX_MERGE_SIZE * multiplier) {
                const result = mergeShapes(a, b, rng);
