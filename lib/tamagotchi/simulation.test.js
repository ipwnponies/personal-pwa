...
describe('applyElapsed', () => {
  ...
  it('sets adultForm on the child->adult transition', () => {
    // Pet that has been in child stage and well-met long enough to transition
    // lastSeen and wellMetSince are set to the past so now - wellMetSince >= STAGE_DURATIONS_MS.child
    // but we apply minimal elapsed time to keep needs above threshold
    ...
    const pet = {
      ...
      lastSeen: childStartTime,
    };
    // now is set such that the duration condition is met, but apply only 1 minute elapsed
    const now = childStartTime + STAGE_DURATIONS_MS.child;
    const elapsed = 60 * 1000; // 1 minute of elapsed time (keeps needs above 60)
    const next = applyElapsed(pet, elapsed, now);
