...
// Rate at which a moving shape is expected to cover ground: one wall thickness
// per second, far less than even the slowest drifting shape manages. The stuck
// threshold is this rate times the window actually sampled, so a short
// stuckAfterS (the tuning panel allows 0.5) gets a proportionally smaller
// threshold rather than a flat one that would false-flag a normally-drifting
// shape as stuck. Structural, so it stays plain.
export const STUCK_DISPLACEMENT_PER_S = WALL_RADIUS * 2;
...
export function trackStuck(
  ...
  shapes.forEach((o) => {
    ...
    if (before && contacted.has(o.id)
      && Math.hypot(o.x - before.x, o.y - before.y) < STUCK_DISPLACEMENT_PER_S * stuckAfterS) {
      stuck.add(o.id);
    }
  });
