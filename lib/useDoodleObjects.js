...
export function useDoodleObjects(rng = Math.random) {
  ...
  // applying (tilt gravity and its damping), plus the wall-collision tuning.
  // Omitted or empty means plain drift, byte-for-byte as before.
  const advance = useCallback((dtSeconds, bounds, grabbedIds, physics = {}) => {
    ...
    const drifted = prev.map((o) => (
      o.kind === 'shape' && !grabbedIds?.has(o.id)
        ? advanceShape(o, dtSeconds, bounds, accel, damping, maxSpeed)
        : o
    ));
