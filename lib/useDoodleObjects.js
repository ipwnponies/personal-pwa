...
export function useDoodleObjects(rng = Math.random) {
  ...
  // applying (tilt gravity and its damping, the gravity well), plus the
  // wall-collision tuning. Omitted or empty means plain drift, byte-for-byte
  // as before.
  const advance = useCallback((dtSeconds, bounds, grabbedIds, physics = {}) => {
    const {
      ...
      accel, damping, maxSpeed, well = null,
    } = physics;
    ...
    // ownership of position integration and the edge bounce (and of the
    // tilt-gravity accel/damping/maxSpeed already threaded through it).
    const drifted = prev.map((o) => {
      ...
      const pulled = well
        ...
        : o;
      return advanceShape(pulled, dtSeconds, bounds, accel, damping, maxSpeed);
    });
