...
// Fast enough that a freshly full creature (100) crosses MET_THRESHOLD (60)
// in well under 4 real minutes — at the old 0.05/min rate that crossing took
// 13+ hours, so seek-eligibility (and the fish-approach-food "magnetism" it
// drives) was unreachable within any normal play session.
export const HUNGER_DECAY_PER_MIN = 12;
export const HAPPINESS_DECAY_PER_MIN = 12;
...
// Scaled down to match the faster hunger/happiness decay above: growth now
// requires actually keeping a creature fed/played-with across the window
// (active care), rather than happening for free over a multi-hour real-time
// span regardless of interaction.
export const STAGE_DURATIONS_MS = { baby: 15 * 60e3, child: 30 * 60e3 };
...
export const applyElapsed = (state, elapsedMs, now = Date.now(), rng = Math.random) => {
  ...
  // Bucketed fish run through the identical step: a fish waiting in the bucket
  // is simply a creature nobody is currently caring for, not one in stasis, so
  // it decays (and grows) exactly like one still in the tank.
  const creatures = state.creatures.map(stepCreature);
  const bucket = state.bucket.map(stepCreature);

  let next = {
    ...state, tankCleanliness, creatures, bucket, lastSeen: now,
  };
