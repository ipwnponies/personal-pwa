...
// A bucketed fish is one nobody is currently caring for, not one in stasis
// (per the fishing design's non-goals), so it runs through exactly the same
// per-creature step as an in-tank fish.
describe('applyElapsed for bucketed fish', () => {
  const withBucketedTwin = (tank) => ({
    ...tank,
    bucket: [{ ...tank.creatures[0], id: 'bucketed' }],
  });

  it('decays a bucketed fish exactly like an in-tank one', () => {
    const tank = withBucketedTwin(createDefaultTank(0, () => 0.5));
    const next = applyElapsed(tank, 3 * MIN, 3 * MIN);
    expect(next.bucket[0].hunger).toBeLessThan(NEED_MAX);
    expect(next.bucket[0].happiness).toBeLessThan(NEED_MAX);
    expect(next.bucket[0].hunger).toBe(next.creatures[0].hunger);
    expect(next.bucket[0].happiness).toBe(next.creatures[0].happiness);
  });

  it('applies the dirty-tank happiness drag to a bucketed fish too', () => {
    const tank = { ...withBucketedTwin(createDefaultTank(0, () => 0.5)), tankCleanliness: 20 };
    const next = applyElapsed(tank, 3 * MIN, 3 * MIN);
    expect(next.tankCleanliness).toBeLessThan(MET_THRESHOLD);
    expect(next.bucket[0].happiness).toBe(next.creatures[0].happiness);
    expect(next.bucket[0].happiness).toBeLessThan(next.bucket[0].hunger);
  });

  it('grows a bucketed fish kept fed/played-with across the stage duration', () => {
    let tank = withBucketedTwin(createDefaultTank(0, () => 0.5));
    const dur = STAGE_DURATIONS_MS.baby;
    let elapsed = 0;
    while (elapsed < dur + MIN) {
      elapsed += MIN;
      tank = applyElapsed(tank, MIN, elapsed);
      const topUp = (c) => ({ ...c, hunger: NEED_MAX, happiness: NEED_MAX });
      tank = { ...tank, creatures: tank.creatures.map(topUp), bucket: tank.bucket.map(topUp) };
    }
    expect(tank.bucket[0].stage).toBe('child');
    expect(tank.bucket[0].stage).toBe(tank.creatures[0].stage);
  });

  it('never lets a bucketed fish fall below the need floor', () => {
    const tank = withBucketedTwin(createDefaultTank(0, () => 0.5));
    const next = applyElapsed(tank, 1000 * HOUR, 1000 * HOUR);
    expect(next.bucket[0].hunger).toBeGreaterThanOrEqual(NEED_FLOOR);
    expect(next.bucket[0].happiness).toBeGreaterThanOrEqual(NEED_FLOOR);
  });
});
...
describe('assignSeekTargets', () => {
  it('claims the only available food drop for a hungry creature within range', () => {
    let tank = createDefaultTank(0, () => 0.5);
  ...
  it('lets a much closer creature win despite lower affinity (proximity term)', () => {
    ...
    // c1: farther (dist 0.3, still within DETECTION_RADIUS) and hungrier.
    // hunger=25 (not the floor) keeps c1's affinity below the clamp ceiling
    // so the proximity term is what decides this case, not a maxed-out
    // affinity swamping it.
    tank.creatures[1].x = 0.8;
    tank.creatures[1].y = 0.5;
    tank.creatures[1].hunger = 25;
    tank = dropFood(tank, 0.5, 0.5, 1000);
  ...
  it('lets a much closer creature win despite lower affinity, toy/happiness branch (proximity term)', () => {
    // Mirrors the food/hunger proximity test above but exercises the toy/
    // happiness branch of assignSeekTargets' `d.type === 'food' ? c.hunger :
    // c.happiness` ternary, which the food-only tests never touch.
    let tank = createDefaultTank(0, () => 0.5);
    tank.creatures = tank.creatures.map((c, i) => ({ ...c, id: `c${i}` }));
    // c0: close (dist 0.01) but barely unhappy (low affinity).
    tank.creatures[0].x = 0.51;
    tank.creatures[0].y = 0.5;
    tank.creatures[0].happiness = 55;
    // c1: farther (dist 0.3, still within DETECTION_RADIUS) and unhappier.
    // happiness=25 (not the floor) keeps c1's affinity below the clamp
    // ceiling so the proximity term is what decides this case, not a
    // maxed-out affinity swamping it.
    tank.creatures[1].x = 0.8;
    tank.creatures[1].y = 0.5;
    tank.creatures[1].happiness = 25;
    tank = dropToy(tank, 0.5, 0.5, 1000);
    // Constant rng means the random term is identical for both candidates,
    // so it can't be what decides this case.
    const next = assignSeekTargets(tank, () => 0.5);
...
describe('computeAffinity', () => {
  ...
  it('is 1 when the need is fully depleted (below the reachable floor)', () => {
    // 0 is below NEED_FLOOR and never actually reached in play — this is a
    // coincidence of clamping, not proof the formula treats 0 as the max.
    expect(computeAffinity(0)).toBe(1);
  });

  it('is 1 at NEED_FLOOR, the actual maximum affinity reachable in play', () => {
    expect(computeAffinity(NEED_FLOOR)).toBe(1);
  });

  it('scales linearly between the threshold and the reachable floor', () => {
    // Midpoint of the reachable range [NEED_FLOOR, MET_THRESHOLD], not of
    // [0, MET_THRESHOLD] — the range a need can actually occupy in play.
    const midpoint = (MET_THRESHOLD + NEED_FLOOR) / 2;
    expect(computeAffinity(midpoint)).toBeCloseTo(0.5, 5);
  });
