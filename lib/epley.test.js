...
describe('calculatePercentageOfOneRm', () => {
  test('1 rep is 100% of 1RM', () => {
    expect(calculatePercentageOfOneRm(1)).toBe(100);
  });

  test('5 reps is approximately 88.24% of 1RM', () => {
    expect(calculatePercentageOfOneRm(5)).toBeCloseTo(88.235294, 5);
  });

  test('30 reps is approximately 50.85% of 1RM', () => {
    expect(calculatePercentageOfOneRm(30)).toBeCloseTo(50.847458, 5);
  });
});

describe('roundToNearestFivePercent', () => {
  test('rounds down when closer to the lower multiple of 5', () => {
    expect(roundToNearestFivePercent(88.235294)).toBe(90);
  });

  test('rounds to 100 for exactly 100', () => {
    expect(roundToNearestFivePercent(100)).toBe(100);
  });

  test('rounds to 50 for a value just above 50', () => {
    expect(roundToNearestFivePercent(50.847458)).toBe(50);
  });

  test('rounds a mid-range decimal-reps percentage correctly', () => {
    // 12.4 reps: ratio = 1 + 11.4/30 = 1.38, %1RM = 100/1.38 ≈ 72.4638
    expect(roundToNearestFivePercent(calculatePercentageOfOneRm(12.4))).toBe(70);
  });
});
