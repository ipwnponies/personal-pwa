...
describe('resolveCollisions', () => {
  ...
  it('bounces overlapping shapes of different colors, conserving momentum', () => {
    ...
    const b = shape({
      id: 'b', x: 10, y: 0, size: 20, color: '#457b9d', shapeType: 'square', vx: -10, vy: 0,
    });
  ...
  it('merges overlapping shapes that share a shapeType even with different colors', () => {
    const a = shape({
      id: 'a', x: 0, y: 0, size: 20, color: '#e63946', shapeType: 'square',
    });
    const b = shape({
      id: 'b', x: 5, y: 0, size: 20, color: '#457b9d', shapeType: 'square',
    });
    const { shapes, events } = resolveCollisions([a, b]);
    expect(shapes).toHaveLength(1);
    expect(events).toHaveLength(1);
    expect(events[0].type).toBe('merge');
    expect(shapes[0].shapeType).toBe('square'); // both parents share it, no ambiguity
  });

  it('still bounces shapes that share neither color nor shapeType', () => {
    const a = shape({
      id: 'a', x: 0, y: 0, size: 20, color: '#e63946', shapeType: 'circle', vx: 10, vy: 0,
    });
    const b = shape({
      id: 'b', x: 10, y: 0, size: 20, color: '#457b9d', shapeType: 'square', vx: -10, vy: 0,
    });
  ...
  it('threads a custom rng into mergeShapes for deterministic shapeType/color selection', () => {
    const a = shape({
      id: 'a', x: 0, y: 0, size: 20, color: '#e63946', shapeType: 'circle',
    });
    const b = shape({
      id: 'b', x: 5, y: 0, size: 20, color: '#457b9d', shapeType: 'circle',
    });
    const { shapes } = resolveCollisions([a, b], null, () => 0); // < 0.5 every time -> always picks 'a'
    expect(shapes[0].color).toBe('#e63946');
  });
  ...
  it('separates coincident shapes (zero-distance case)', () => {
    // Regression test: two shapes can end up exactly coincident (e.g. two
    // taps landing on the same pixel). Shapes with neither color nor
    // shapeType in common should bounce and separate.
    ...
    const b = shape({
      id: 'b', x: 0, y: 0, size: 20, color: '#457b9d', shapeType: 'square', vx: -10, vy: 0,
    });
    const { shapes, events } = resolveCollisions([a, b]);
    expect(shapes).toHaveLength(2);
    const na = shapes.find((s) => s.id === 'a');
