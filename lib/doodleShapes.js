...
// Cap on merged-shape size (2x the spawn maximum). Merge-eligible shapes at
// or above this combined size bounce instead of merging, so the kid — not
// an auto-pop — decides when a shape explodes (via the existing double-tap).
...
export function splitShape(
  ...
  // Half the child's own radius as spawn padding: children start already
  // partly separated along their emission angle instead of dead-center on
  // the parent, so — combined with SPLIT_GRACE_S and their outward
  // velocity — they have a real chance to disperse before grace expires.
