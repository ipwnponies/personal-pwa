...
// Scales a velocity back to maxSpeed while preserving its direction. A
// non-finite or non-positive maxSpeed (e.g. a tuning-panel field cleared or
// set to 0 mid-edit) is treated as "no cap" rather than degenerate-scaling
// every velocity toward zero — the scale-down math below only ever shrinks,
// so a bad value here would otherwise leave shapes permanently slow/frozen
// even after a valid maxSpeed is entered afterward.
export const clampSpeed = (vx, vy, maxSpeed) => {
  if (!Number.isFinite(maxSpeed) || maxSpeed <= 0) return { vx, vy };
  const speed = Math.hypot(vx, vy);
...
export function mergeShapes(a, b, rng = Math.random) {
  ...
  // Neither parent's wallImmunityRemaining survives the spread below by
  // default, so a merge of two shapes released next to a wall a moment ago
  // would otherwise get an immediate wall bounce on its spawn frame — the
  // same spawn-immunity gap splitShape avoids by always granting
  // splitGraceRemaining to its children (see .claude/rules/doodle.md).
  // Inherit the longer of the two remaining countdowns, if either is active.
  const wallImmunityRemaining = Math.max(a.wallImmunityRemaining || 0, b.wallImmunityRemaining || 0);
  return {
    id: generateId(),
    kind: 'shape',
    ...
    ...(wallImmunityRemaining > 0 && { wallImmunityRemaining }),
  };
}
...
export function advanceShape(shape, dtSeconds, bounds) {
  ...
  // is detected stuck against a wall, and unconditionally on every drag/pinch
  // release — not only on a release that overlaps a wall. Note advanceShape
  // only runs on non-grabbed shapes, so the countdown does not tick while a
  // shape is held — which is what we want: a held shape ignores walls anyway,
  // and the clock should start at release.
