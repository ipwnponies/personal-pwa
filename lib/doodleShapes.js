...
// Split children spawn coincident at the parent's exact x,y, sharing its
// color. Without immunity, resolveCollisions' same-color merge branch fires
// on the very next frame — before they've had time to drift apart — and
// silently recombines them, so a pop looks like nothing happened. This grace
// window exempts a freshly split shape from all collision handling (merge,
// bounce, position correction) until it's had a moment to disperse.
export const SPLIT_GRACE_S = 0.4;
...
export function createShape(x, y, rng = Math.random, sizeMultiplier = 1) {
