...
import { clamp, weightedRandomChoice, generateId, pushHistoryEntry } from './random';
...
describe('pushHistoryEntry', () => {
  it('prepends entry to empty history', () => {
    const result = pushHistoryEntry([], { id: '1', label: 'A' }, 20);
    expect(result).toEqual([{ id: '1', label: 'A' }]);
  });

  it('puts newest entry first', () => {
    const existing = [{ id: '1', label: 'A' }];
    const result = pushHistoryEntry(existing, { id: '2', label: 'B' }, 20);
    expect(result.map((e) => e.id)).toEqual(['2', '1']);
  });

  it('caps result at maxEntries, dropping oldest', () => {
    const existing = [{ id: '2' }, { id: '1' }];
    const result = pushHistoryEntry(existing, { id: '3' }, 2);
    expect(result.map((e) => e.id)).toEqual(['3', '2']);
  });

  it('does not mutate the original array', () => {
    const existing = [{ id: '1' }];
    pushHistoryEntry(existing, { id: '2' }, 20);
