...
import {
  spawnBurst, spawnSpiral, spawnSquashPoof, spawnDust, advanceParticles, DEFAULT_MAX_PARTICLES,
  COLLISION_BURST_MAX_AGE,
...
describe('doodleParticles', () => {
  ...
  it('spawnBurst accepts a longer maxAge for collision feedback', () => {
    const particles = spawnBurst(10, 20, '#e63946', null, COLLISION_BURST_MAX_AGE);
    expect(COLLISION_BURST_MAX_AGE).toBe(1);
    particles.forEach((p) => expect(p.maxAge).toBe(COLLISION_BURST_MAX_AGE));
  });

  it('advanceParticles decays burst-kind velocity (friction) but leaves other kinds alone', () => {
    const particles = [
      {
        id: 'b', kind: 'burst', x: 0, y: 0, vx: 100, vy: 0, color: '#000', age: 0, maxAge: 1,
      },
      {
        id: 'd', kind: 'dust', x: 0, y: 0, vx: 100, vy: 0, color: '#000', age: 0, maxAge: 1,
      },
    ];
    const next = advanceParticles(particles, 0.5);
    const burst = next.find((p) => p.id === 'b');
    const dust = next.find((p) => p.id === 'd');
    expect(burst.vx).toBeLessThan(100); // slowed by friction
    expect(burst.vx).toBeGreaterThan(0); // still moving forward, not reversed
    expect(dust.vx).toBe(100); // unaffected
  });
  ...
  it('spawnDust accepts a custom maxAge', () => {
    const particles = spawnDust(0, 0, 10, 0, '#2a9d8f', 1.5);
    particles.forEach((p) => expect(p.maxAge).toBe(1.5));
  });
  ...
  it('advanceParticles caps the array at DEFAULT_MAX_PARTICLES, dropping the oldest first', () => {
    const particles = Array.from({ length: DEFAULT_MAX_PARTICLES + 10 }, (_, i) => ({
      id: `p${i}`, kind: 'dust', x: 0, y: 0, vx: 0, vy: 0, color: '#000', age: 0, maxAge: 10,
    }));
    const next = advanceParticles(particles, 0.01);
    expect(next).toHaveLength(DEFAULT_MAX_PARTICLES);
    expect(next[0].id).toBe('p10'); // the first 10 (oldest) were dropped
  });

  it('advanceParticles honors a custom maxParticles cap', () => {
    const particles = Array.from({ length: 15 }, (_, i) => ({
      id: `p${i}`, kind: 'dust', x: 0, y: 0, vx: 0, vy: 0, color: '#000', age: 0, maxAge: 10,
    }));
    const next = advanceParticles(particles, 0.01, 5);
    expect(next).toHaveLength(5);
    expect(next[0].id).toBe('p10');
  });
});
